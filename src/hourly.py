                ...
                if (a["metrics"]==[]):
                       ...
                       cur.execute("Insert into hourly Values (%d,%50s,%10s,%d,%50s,%s,%s,%s,%s,%s,%s,%s);" % (c,repr(day_nr),repr('N/A'),(res_list[id_v]),repr(res_name[0]),repr('N/A'), repr('N/A'),repr('N/A'),repr('N/A'),repr('N/A'),repr('N/A'),repr('NR')))
                        ...
                        for key_date, value in result.items():
                                    ...
                                    day=day.replace(','," ")
                                    day_str=str(datetime.strptime(day, '%b %d %Y'))
                                    lyst_day= day_str.split()
                                    day_iso= lyst_day[0]
                                    #print(day_iso)
                                    hr=lyst_date[3]
                                    lyst_hr=hr.split(':')
                                    ...
                                    res_len = [x[0] for x in len_tab]
                                    c= (res_len[0]+1)                                           
                                    cur.execute("Insert into hourly Values (%d,%50s,%10s,%d,%50s,%s,%s,%s,%s,%s,%s,%s);" % (c,repr(day_iso),repr(lyst_hr[0]),(res_list[id_v]),repr(res_name[0]),repr(PM10_v),repr(SO2_v),repr(NO2_v),repr(PM25_v),repr(CO_v),repr(O3_v),repr(stat)))
